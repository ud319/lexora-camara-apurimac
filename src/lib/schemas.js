...
function docSchema(prefix) {
  return z.object({
    ...
    razon_social: z.string().min(1, 'Ingrese la razón social del demandado'),
    ...
    if (val.tipo_persona === 'natural' && !val.nombres?.trim()) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Ingrese el nombre completo del demandado', path: ['nombres'] })
    }
    if (val.tipo_persona === 'juridica' && !val.razon_social?.trim()) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Ingrese la razón social del demandado', path: ['razon_social'] })
    }
  })
}
